...
PDC Lab Exam | High-Performance Cyberpunk Implementation
Aesthetic: Dark Obsidian & Glassmorphism — Neon Cyan + Electric Pink + Lime Glow
"""
...
# ─────────────────────────────────────────────────────────────────────────────
#  HIGH-CONTRAST CYBERPUNK PALETTE & STYLES
# ─────────────────────────────────────────────────────────────────────────────
BG          = "#03070d"
BG2         = "#081018"
PANEL_BG    = "#050b12"
...
# ─────────────────────────────────────────────────────────────────────────────
#  NEON PANEL CANVAS (Notched corners + glowing border)
# ─────────────────────────────────────────────────────────────────────────────
class NeonPanel(tk.Canvas):
    ...
    def _redraw(self, e=None):
        ...
        # Subtle outer glow & crisp inner border
        self.create_polygon(pts, outline=c, fill=BG2, width=1, tags="border")
        self.create_polygon(pts, outline=c, fill="", width=2, stipple="gray25", tags="border")
        
        if self._label:
            self.create_rectangle(10, 0, 14 + len(self._label)*7, 14, fill=BG2, outline="", tags="border")
            self.create_text(12, 0, text=f" {self._label} ", anchor="nw",
                             font=FONT_MONO, fill=c, tags="border")
...
class WaveformCanvas(tk.Canvas):
    ...
    def _draw(self):
        ...
        for j in range(0, w, max(20, w // 8)):
            self.create_line(j, 0, j, h, fill=GRID_C, width=1)
...
# ─────────────────────────────────────────────────────────────────────────────
#  NEON BAR (Sleek hardware usage bar)
# ─────────────────────────────────────────────────────────────────────────────
class NeonBar(tk.Canvas):
    ...
    def _draw(self):
        ...
        w, h = self.winfo_width(), self.winfo_height()
        if w < 2: return
        self.create_rectangle(0, 0, w, h, fill="#081420", outline="")
        fw = int(w * self._pct / 100)
        col = pct_color(self._pct)
        if fw > 0:
            self.create_rectangle(0, 0, fw, h, fill=col, outline="")
            self.create_rectangle(0, 0, fw, max(1, h//2), fill="#ffffff", stipple="gray25", outline="")
...
class MonitorTab(tk.Frame):
    ...
    def _build_ui(self):
        top = tk.Frame(self, bg=BG, height=32)
        top.pack(fill=tk.X, padx=8, pady=(6, 2))
        top.pack_propagate(False)

        self._stress_btn = tk.Button(
            top, text="⚡ Launch CPU Stress Test (10s)",
            font=FONT_HEAD, bg=PINK, fg="#ffffff",
            activebackground="#ff3377", activeforeground="#ffffff",
            relief=tk.FLAT, cursor="hand2", bd=0, padx=12, pady=2,
            command=self._do_stress)
        self._stress_btn.pack(side=tk.LEFT, padx=4)

        self._stress_lbl = tk.Label(top, text="● STRESS ACTIVE",
                                     font=FONT_HEAD, fg=RED_N, bg=BG)
        ...
        # Left Column: Stat Cards + CPU Cores
        ...
        mem_p = NeonPanel(cards, color=PINK, label="MEMORY", height=60)
        mem_p.grid(row=0, column=0, padx=2, sticky="nsew")
        self._mem_val = tk.Label(mem_p, text="– MB", font=FONT_MED, fg=PINK, bg=BG2)
        self._mem_val.place(relx=0.5, rely=0.55, anchor="center")

        cpu_p = NeonPanel(cards, color=CYAN, label="CPU OVERALL", height=60)
        cpu_p.grid(row=0, column=1, padx=2, sticky="nsew")
        self._cpu_card = tk.Label(cpu_p, text="0%", font=FONT_MED, fg=CYAN, bg=BG2)
        self._cpu_card.place(relx=0.5, rely=0.55, anchor="center")

        sys_p = NeonPanel(cards, color=GREEN_N, label="SYSTEM", height=60)
        sys_p.grid(row=0, column=2, padx=2, sticky="nsew")
        self._procs_lbl = tk.Label(sys_p, text="– procs", font=FONT_MED, fg=GREEN_N, bg=BG2)
        self._procs_lbl.place(relx=0.5, rely=0.55, anchor="center")

        # Core bars container
        cores_p = NeonPanel(left, color=CYAN, label="CPU CORES")
        cores_p.pack(fill=tk.BOTH, expand=True)

        inner = tk.Frame(cores_p, bg=BG2)
        inner.place(relx=0, rely=0, relwidth=1, relheight=1, x=6, y=20, width=-12, height=-26)
        ...
        for i in range(self._core_count):
            ...
            tk.Label(rf, text=f"Core {i:02d}", font=FONT_MONO, fg=TXT_DIM, bg=BG2,
                     width=7, anchor="w").pack(side=tk.LEFT)
            bar = NeonBar(rf, color=CYAN, height=10)
            bar.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)
            lbl = tk.Label(rf, text="  0.0%", font=FONT_MONO, fg=CYAN, bg=BG2, width=7)
            lbl.pack(side=tk.RIGHT)
            self._c_bars.append(bar)
            self._c_lbls.append(lbl)

        # Right Column: CPU Waveform + RAM Graph + GPU
        ...
        # CPU History Chart
        cpu_hist = NeonPanel(right, color=CYAN, label="REAL-TIME CPU HISTORY")
        cpu_hist.grid(row=0, column=0, sticky="nsew", pady=(0, 6))
        self._cpu_big = tk.Label(cpu_hist, text="0%", font=FONT_BIG, fg=CYAN, bg=BG2)
        self._cpu_big.place(relx=0.04, rely=0.15)
        tk.Label(cpu_hist, text="Avg Usage", font=FONT_SM, fg=TXT_DIM, bg=BG2).place(relx=0.04, rely=0.65)
        
        self._cpu_wave = WaveformCanvas(cpu_hist, color=CYAN, bg=BG2)
        self._cpu_wave.place(relx=0.28, rely=0.08, relwidth=0.70, relheight=0.88)

        # Memory Details
        mem_panel = NeonPanel(right, color=PINK, label="MEMORY ALLOCATION")
        mem_panel.grid(row=1, column=0, sticky="nsew", pady=(0, 6))
        self._mem_big = tk.Label(mem_panel, text="0%", font=FONT_BIG, fg=PINK, bg=BG2)
        self._mem_big.place(relx=0.04, rely=0.12)
        self._mem_detail = tk.Label(mem_panel, text="", font=FONT_MONO, fg=TXT, bg=BG2, justify=tk.LEFT)
        self._mem_detail.place(relx=0.35, rely=0.18)
        self._mem_bar = NeonBar(mem_panel, color=PINK, height=18)
        self._mem_bar.place(relx=0.04, rely=0.68, relwidth=0.92, height=18)

        # GPU Status
        gpu_panel = NeonPanel(right, color=GREEN_N, label="GPU GRAPHICS ADAPTER")
        gpu_panel.grid(row=2, column=0, sticky="nsew")
        self._gpu_big = tk.Label(gpu_panel, text="ACTIVE", font=FONT_HEAD, fg=GREEN_N, bg=BG2)
        self._gpu_big.place(relx=0.04, rely=0.18)
        self._gpu_name = tk.Label(gpu_panel, text=self._detect_gpu(),
                                   font=FONT_MONO, fg=TXT_DIM, bg=BG2)
        self._gpu_name.place(relx=0.25, rely=0.20)
        self._gpu_bar = NeonBar(gpu_panel, color=GREEN_N, height=10)
    ...
    def _detect_gpu(self):
        try:
            ...
            return lines[0][:42] if lines else "Integrated Windows Display Adapter"
        except Exception:
    ...
    def _apply(self, d):
        ...
        self._mem_detail.config(
            text=f"RAM Used:  {used_mb:.0f} MB / {self._total_ram:.0f} MB\n"
                 f"Available: {(self._total_ram - used_mb):.0f} MB")
...
class ProcessTab(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(top, text="FILTER:", font=FONT_HEAD, fg=TXT_DIM, bg=BG).pack(side=tk.LEFT)
        ...
        tk.Entry(top, textvariable=self._fvar, font=FONT_MONO,
                 bg="#071420", fg=CYAN, insertbackground=CYAN,
                 relief=tk.FLAT, highlightthickness=1, highlightbackground=CYAN,
                 width=20).pack(side=tk.LEFT, padx=6)

        tk.Label(top, text="LAUNCH PATH:", font=FONT_HEAD, fg=TXT_DIM, bg=BG).pack(side=tk.LEFT, padx=(10, 0))
        self._pvar = tk.StringVar()
        tk.Entry(top, textvariable=self._pvar, font=FONT_MONO,
                 bg="#071420", fg=PINK, insertbackground=PINK,
                 relief=tk.FLAT, highlightthickness=1, highlightbackground=PINK,
                 width=34).pack(side=tk.LEFT, padx=4)

        for lbl, cmd, col in [("Browse...", self._browse, TXT_DIM),
                                ("Launch ▶", self._launch, GREEN_N)]:
            tk.Button(top, text=lbl, font=FONT_HEAD, bg="#071420", fg=col,
                      activebackground="#0e2436", activeforeground=col,
                      relief=tk.FLAT, cursor="hand2", highlightthickness=1,
                      highlightbackground=col, command=cmd, padx=8).pack(side=tk.LEFT, padx=3)

        self._cnt_lbl = tk.Label(top, text="", font=FONT_MONO, fg=TXT_DIM, bg=BG)
        self._cnt_lbl.pack(side=tk.RIGHT, padx=8)

        # Style Treeview
        style = ttk.Style()
        style.configure("P.Treeview", background=PANEL_BG, foreground=TXT,
                         fieldbackground=PANEL_BG, rowheight=22,
                         font=FONT_MONO, borderwidth=0)
        style.configure("P.Treeview.Heading", background="#0a1a28", foreground=CYAN,
                         font=FONT_HEAD, relief="flat")
        style.map("P.Treeview", background=[("selected", "#0e2c44")],
                  foreground=[("selected", CYAN)])

        tf = tk.Frame(self, bg=BG, highlightbackground=CYAN, highlightthickness=1)
        ...
        for c, h, w in [("name", "Process Name", 240), ("pid", "PID", 80),
        ...
        for lbl, cmd, col in [("⏸ SUSPEND PROCESS", self._suspend, YELLOW_N),
                                ("▶ RESUME PROCESS",  self._resume,  GREEN_N),
                                ("✕ KILL PROCESS",    self._kill,    RED_N)]:
            tk.Button(btn_f, text=lbl, font=FONT_HEAD, bg="#071420", fg=col,
                      activebackground="#0e2436", activeforeground=col,
                      relief=tk.FLAT, cursor="hand2", highlightthickness=1,
                      highlightbackground=col, command=cmd, padx=12, pady=4).pack(side=tk.LEFT, padx=6)

        self._status_lbl = tk.Label(self, text="Ready", font=FONT_MONO, fg=TXT_DIM, bg=BG)
        self._status_lbl.pack(anchor="w", padx=10, pady=(0, 2))

    def _start_thread(self):
        # Non-blocking warm up in background
        def init_and_loop():
    ...
    def _enumerate(self):
        # Fast parallel process iteration using ThreadPoolExecutor
    ...
    def _launch(self):
        ...
        try:
            ...
            self._status_lbl.config(text=f"Launched: {os.path.basename(path)}", fg=GREEN_N)
    ...
    def _suspend(self):
        ...
        try:
            ...
            p.suspend()
            self._status_lbl.config(text=f"Suspended Process PID {pid}", fg=YELLOW_N)
    ...
    def _resume(self):
        ...
        try:
            ...
            self._status_lbl.config(text=f"Resumed Process PID {pid}", fg=GREEN_N)
        except Exception as e:
            messagebox.showerror("SysForge Error", str(e))
    ...
    def _kill(self):
        ...
        try:
            ...
            if not messagebox.askyesno("Confirm Terminate", f"Are you sure you want to kill {p.name()} (PID {pid})?"):
                return
            p.kill()
            self._status_lbl.config(text=f"Killed Process PID {pid}", fg=RED_N)
        except psutil.NoSuchProcess:
            self._status_lbl.config(text="Process already terminated.", fg=TXT_DIM)
        except Exception as e:
            messagebox.showerror("SysForge Error", str(e))


# ═════════════════════════════════════════════════════════════════════════════
#  TAB 3  —  PARALLEL SYSTEM CLEANER (High-Speed os.scandir Engine)
# ═════════════════════════════════════════════════════════════════════════════
TARGETS = [
...
class CleanerTab(tk.Frame):
    ...
    def _build_ui(self):
        ...
        self._scan_btn = tk.Button(
            top, text="🔍 SCAN SYSTEM CACHE", font=FONT_HEAD,
            bg="#071420", fg=CYAN, activebackground="#0e2436", activeforeground=CYAN,
            relief=tk.FLAT, cursor="hand2", highlightthickness=1,
            highlightbackground=CYAN, padx=12, pady=2,
            command=self._start_scan)
        self._scan_btn.pack(side=tk.LEFT, padx=4)

        self._prog_lbl = tk.Label(top, text="Idle — click Scan to begin parallel search",
                                   font=FONT_MONO, fg=TXT_DIM, bg=BG)
        self._prog_lbl.pack(side=tk.LEFT, padx=8)

        self._time_lbl = tk.Label(top, text="", font=FONT_MONO, fg=GREEN_N, bg=BG)
        ...
        self._summ = tk.Label(self, text="", font=FONT_MONO, fg=TXT, bg=BG, justify=tk.LEFT)
        self._summ.pack(anchor="w", padx=10, pady=2)

        # Style Treeview
        style = ttk.Style()
        style.configure("C.Treeview", background=PANEL_BG, foreground=TXT,
                         fieldbackground=PANEL_BG, rowheight=20,
                         font=FONT_MONO, borderwidth=0)
        style.configure("C.Treeview.Heading", background="#0a1a28", foreground=PINK,
                         font=FONT_HEAD)
        style.map("C.Treeview", background=[("selected", "#0e2c44")],
                  foreground=[("selected", PINK)])

        tf = tk.Frame(self, bg=BG, highlightbackground=PINK, highlightthickness=1)
        tf.pack(fill=tk.BOTH, expand=True, padx=8, pady=(0, 4))

        cols = ("sel", "path", "size", "mtime", "cat")
        self._tree = ttk.Treeview(tf, columns=cols, show="headings", style="C.Treeview")
        for c, h, w in [("sel", "✓", 30), ("path", "Full File Path", 420),
        ...
        for lbl, cmd, col in [("☑ Select All", self._sel_all, TXT_DIM),
                                ("☐ Deselect All", self._desel_all, TXT_DIM)]:
            tk.Button(bot, text=lbl, font=FONT_MONO, bg="#071420", fg=col,
                      activebackground="#0e2436", relief=tk.FLAT,
        ...
        self._del_btn = tk.Button(
            ...
            bg="#071420", fg=RED_N, activebackground="#0e2436", activeforeground=RED_N,
            relief=tk.FLAT, cursor="hand2", highlightthickness=1,
            highlightbackground=RED_N, command=self._delete, padx=12, pady=2)
    ...
    def _start_scan(self):
        ...
        self._pbar.start(10)
        self._prog_lbl.config(text="Scanning directories in parallel...", fg=YELLOW_N)
    ...
    def _scan_worker(self):
        ...
        # Fast recursive scandir function
        ...
        # Launch 7 parallel worker threads (one per target directory)
    ...
    def _scan_done(self, files, el):
        ...
        self._prog_lbl.config(text=f"Scan Complete — {len(files)} files found", fg=GREEN_N)
    ...
    def _update_summary(self):
        ...
        self._summ.config(text=f"Found: {len(self._files)} files ({tot/1024:.1f} MB total)   [{parts}]")
...
class ProofPanel(tk.Toplevel):
    def __init__(self, master):
        ...
        self.geometry("560x340+20+20")
        self.protocol("WM_DELETE_WINDOW", self.withdraw)

        hdr = tk.Frame(self, bg=BG, highlightbackground=CYAN, highlightthickness=1)
        ...
        self._txt = tk.Text(self, font=("Courier New", 8), bg=BG2, fg=GREEN_N,
                             relief=tk.FLAT, width=70, height=16, state=tk.DISABLED,
    ...
    def _tick(self):
        ...
        lines = [
            ...
            "",
            "MONITOR DATA SOURCE",
            f"  API Used       : psutil (cpu_percent + virtual_memory)",
            ...
            f"  PIDs Scanned   : {d['process_pids_scanned']} processes",
            f"  Scan Duration  : {d['process_scan_ms']:.0f} ms",
            "",
            "CLEANER ENGINE TELEMETRY",
            f"  Directory Threads: {d['cleaner_threads']} concurrent threads",
            f"  Last Scan Time : {d['cleaner_scan_time']:.2f} seconds",
...
# ═════════════════════════════════════════════════════════════════════════════
#  MAIN WINDOW WITH CUSTOM HIGH-TECH TAB NAV BAR
# ═════════════════════════════════════════════════════════════════════════════
class SysForge(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("SYSFORGE — Real-Time Windows System Utility")
        self.configure(bg=BG)
    ...
    def _build(self):
        # Top Title Bar
        tb = tk.Frame(self, bg=BG, height=40)
        tb.pack(fill=tk.X)
        tb.pack_propagate(False)

        tk.Label(tb, text="◈ SYS", font=("Courier New", 16, "bold"), fg=CYAN, bg=BG).pack(side=tk.LEFT, padx=(12, 0))
        tk.Label(tb, text=":FORGE CORE", font=("Courier New", 16, "bold"), fg=PINK, bg=BG).pack(side=tk.LEFT)
        tk.Label(tb, text="  //  PARALLEL WINDOWS SYSTEM UTILITY", font=FONT_MONO, fg=TXT_DIM, bg=BG).pack(side=tk.LEFT, pady=6)

        self._clock = tk.Label(tb, text="", font=FONT_HEAD, fg=CYAN, bg=BG)
        self._clock.pack(side=tk.RIGHT, padx=14)
        tk.Label(tb, text="[ F2 ] PROOF PANEL", font=FONT_MONO, fg=TXT_DIM, bg=BG).pack(side=tk.RIGHT, padx=8)
        self._tick_clock()
        ...
        # Custom Cyberpunk Tab Bar Container
        nav_frame = tk.Frame(self, bg="#050d17", height=38)
        ...
        tab_names = ["  ◈ HARDWARE MONITOR  ", "  ◈ PROCESS CONTROLLER  ", "  ◈ SYSTEM CLEANER  "]
        for idx, name in enumerate(tab_names):
            btn = tk.Button(nav_frame, text=name, font=FONT_HEAD,
                            bg="#050d17", fg=TXT_DIM, activebackground="#0a1a2a", activeforeground=CYAN,
                            bd=0, relief=tk.FLAT, cursor="hand2", padx=16,
    ...
    def _select_tab(self, index):
        for idx, tab in enumerate(self._tabs):
            if idx == index:
                ...
                self._tab_btns[idx].config(bg=BG, fg=CYAN)
            else:
                tab.pack_forget()
                self._tab_btns[idx].config(bg="#050d17", fg=TXT_DIM)
